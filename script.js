...
let scores, currentScore, activePlayer, playing, winningScore, playerNames;
...
btnStart.addEventListener('click', function () {
  const p1 = inputPlayer1.value.trim() || 'Player 1';
  const p2 = inputPlayer2.value.trim() || 'Player 2';
  const win = parseInt(inputScore.value) || 100;
  ...
  if (playing) {
    ...
    if (scores[activePlayer] >= winningScore) {
      ...
      document.getElementById(
        `name--${activePlayer}`
      ).textContent = `${playerNames[activePlayer]} Wins! 🎉`;
    } else {
      switchPlayer();
    }
  }
